...
/**
 * ⚡ Bolt Optimization:
 * Price APIs now use pre-serialized JSON and pre-calculated ETags.
 * This bypasses JSON.stringify() and hashing overhead on every request.
 */
app.get('/api/prices/metals', async (req, res) => {
    try {
        await fetchMetalPrices();
        const { json, etag } = getMetalCache();
...
app.get('/api/prices/crypto', async (req, res) => {
    try {
        ...
        const { json, etag } = getCryptoCache();
