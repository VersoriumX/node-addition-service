 ...
 * ⚡ Bolt Optimization: Highly optimized string comparison and prefix checks
 * to bypass heavy regex replacement and .trim() allocations on hot path.
 */
function isETagMatch(reqHeader, etag) {
    if (!reqHeader) return false;
    if (reqHeader === etag) return true;

    // Fast prefix slice checks instead of costly regex replacement
