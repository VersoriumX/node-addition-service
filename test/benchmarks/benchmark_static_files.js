...
// 1. Dynamic path (simulating dynamic disk read + hashing)
function runDynamicRead() {
    ...
    const etagRareearth = crypto.createHash('md5').update(rareearth).digest('hex');
    const etagIsolation = crypto.createHash('md5').update(isolation).digest('hex');
    return { rareearth, isolation, etagRareearth, etagIsolation };
}

// 2. Optimized path: Eagerly cached on startup (references to in-memory buffers/strings)
...
console.log(`Time saved per ${ITERATIONS.toLocaleString()} requests: ${(dynamicTime - cached304Time).toFixed(2)}ms`);
