...
function checkObject(obj, depth = 0) {
    ...
    for (const key in obj) {
        ...
        if (typeof val === 'string') {
            // ⚡ Bolt Optimization: Simple length check - common ReDoS payloads are often very long
...
function electricFence(req, res, next) {
    ...
    // ⚡ Bolt Optimization: Directly pass objects to checkObject to avoid Object.keys overhead.
    let isSuspicious = checkObject(req.query) || checkObject(req.body);
