...
## 2026-07-26 - Firewall Bypass via JSON Keys
**Vulnerability:** The recursive `checkObject` helper inside the `electricFence` security middleware only scanned object values for malicious patterns (like globstars or extremely long strings), leaving object keys completely uninspected and creating a major bypass vector.
**Learning:** Malicious inputs can enter an application through JSON keys as well as values. Security middleware must inspect the entire surface area of incoming request data (both keys and values) to prevent parsing-related vulnerabilities or ReDoS attacks.
**Prevention:** Always inspect object keys in recursive payload scanners to ensure comprehensive firewall coverage and defense-in-depth.
