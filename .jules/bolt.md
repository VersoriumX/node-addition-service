...
## 2026-07-07 - Lazy Initialization of Cryptographic Keys to Prevent Startup Lag
**Learning:** Synchronously generating cryptographically secure RSA keys (like 2048-bit keypairs) during module import / load time introduces significant, synchronous blocking of the event loop (~80ms+), which directly degrades server startup latency and container spin-up times.
**Action:** Defer expensive cryptographic operations, such as RSA key generation or cipher loading, until the actual functions are invoked (lazy initialization), keeping the module loading footprint minimal (<20ms).
