...
## 2026-09-02 - Throttled Map Pruning & Avoiding Redundant Map Set Operations in Rate Limiter Middleware
**Learning:** In high-frequency security/rate-limiting middleware, invoking a full-map iteration (`for...of`) whenever a Map size threshold is met can cause catastrophic performance degradation ($O(N)$ per request) if active entries have not yet expired. Additionally, re-inserting mutated object references into a JS `Map` (`map.set(key, val)`) adds unnecessary hash table lookup overhead.
**Action:** Throttle full-map stale-entry pruning using a timestamp delta check (`now - lastPruneTime >= 1000`) so full-map scans run at most once per second during high load. Mutate existing object entries in-place without calling `map.set()` again. This reduces execution time for 500,000 requests from >400 seconds (timeout) down to ~885ms (~99.8% speedup).
