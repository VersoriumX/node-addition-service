...
## 2026-08-30 - In-Memory Fuzzer Cache to Avoid Repeated Allocations and CPU Overhead
**Learning:** Generating deterministic fuzzer variations involves extensive string manipulation (casing, reversals, regex replacements) and frequent object/Set/Array allocations. For high-frequency query paths, these operations generate significant CPU and GC overhead. Implementing a size-limited, TTL-backed cache completely eliminates these redundant operations.
**Action:** Use a private, size-limited, and TTL-backed Map cache inside helper/fuzzer modules to cache heavy deterministic transformations of user inputs, ensuring memory safety with FIFO eviction.
