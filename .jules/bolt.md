...
## 2026-07-23 - Express Middleware Pipeline Reordering for Static Routes
**Learning:** Placing body parsers (`express.json()`) and computationally intensive security scanners/validation middlewares (like recursive ReDoS payload validation) on the global middleware stack before static asset routing (`express.static`) introduces massive, unnecessary CPU overhead and memory allocation for purely static GET requests. Since static endpoints do not read query parameters or JSON body payloads for application processing, they do not present injection/ReDoS surfaces.
**Action:** Always route purely static file folders (`express.static`) and purely static GET endpoints (e.g. `robots.txt`, home pages) before mounting heavy global request-parsing, session-management, or recursive payload-scanning middlewares. Bypassing these middlewares on static assets speeds up static content delivery overhead by ~95% and reduces unnecessary garbage collection.
